1. Why we use unsafe only for cyclic check? can the real ptr be changed by garbage collector?
- maybe the unsafe.Pointer will change as variable move around, unlike the uinptr return by UnsafeAddress()
- And we use unsafe.Pointer because it can hold any type of pointer!

2. we need types and addresses as different variables can have the same addresses as x and x[0]

- the flow is for the most simplified case of 2 string slice. similar to int slice
